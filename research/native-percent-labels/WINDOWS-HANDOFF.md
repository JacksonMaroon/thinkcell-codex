...
This is an evidence-only research branch. It does not add percentage conversion support.
...
- [ ] Record Windows, PowerPoint, think-cell, and Python versions.
- [ ] Run the two checks above and record the result.
- [ ] Copy decks 02 through 05 to a separate working folder before opening them in PowerPoint with think-cell enabled.
- [ ] Confirm the Series 3 / 2024 label reads 50%, 67%, 50%, 50% respectively.
...
- [ ] Add explicit discovery for direct-rendered native labels, reporting direct precision, owning chart, and category/series identity.
- [ ] Keep field-backed selection guards intact. A missing relative text variable is not sufficient to classify a label as static.
- [ ] Check native ownership, axis/data semantics, and numerical agreement together. Neither the percent suffix nor the MSGraph-rendering flag proves dynamic percentages on its own.
...
- [ ] Keep discovery support separate from permission to mutate a label.
...
- [ ] Test official JSON / `.ppttc` regeneration independently from datasheet edits.
...
The Mac experiment proved default native percentage labels remain dynamic. It did not prove absolute-to-percent conversion, Windows COM behavior, or JSON regeneration. See [README.md](README.md) for the observed structures and exact limitations.
