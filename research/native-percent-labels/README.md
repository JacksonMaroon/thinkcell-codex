...
Date: 2026-09-21. Mac think-cell 14.0.38775, PowerPoint 16.113.1. Synthetic native charts only. Production code is unchanged.
...
- The native chart part contains normalized values on a 0–100 scale. Its data labels use `c:showVal=1`, `c:showPercent=0`, and number formatting with a literal percent suffix. No `a:fld` appears in that chart part.
...
Files 00 and 01 were independently inserted through the chart gallery. They are not a controlled single-setting conversion pair. Files 02–05 are successive versions of the same percentage chart. Labels on the other two categories and their underlying values remain unchanged. The final screenshot is `04-reopened.png`.
...
1. Add an explicit direct-rendered-label discovery branch alongside the existing field-backed branch. Preserve the existing branch's guards. Resolve chart ownership through relationships and tags, and map category/series identities rather than assuming reverse order globally.
2. Extend read-only label inspection to report direct `m_prec` and native chart label formats. Add this corpus as a regression fixture. A null relative text variable should not be reported as universally non-dynamic.
3. Establish a same-chart UI conversion pair and reproduce it on a Windows build before attempting a production mutation adapter. Test official JSON regeneration separately from datasheet editing.

## Remaining limitations

The floating Label Content toolbar could not be captured reliably through the available Mac computer-use interface. Therefore absolute-to-percent conversion, combined labels, custom formats, and removal of the existing workaround remain unproven. This experiment used default native percentage labels instead. Windows COM, cross-platform portability and `.ppttc` regeneration were not tested. Do not patch production labels by copying chart-cache formatting or removing validation checks.
